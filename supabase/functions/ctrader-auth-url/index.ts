...
serve(async (req) => {
  ...
  try {
    ...
    const authUrl = `https://openapi.ctrader.com/apps/auth?client_id=${encodeURIComponent(clientId)}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=code&scope=${encodeURIComponent(scope)}&state=${encodeURIComponent(userId)}`;
    
    console.log('🔗 Generated cTrader auth URL for user:', userId);
