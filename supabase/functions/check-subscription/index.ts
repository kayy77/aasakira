...
serve(async (req) => {
  ...
  try {
    ...
    // First check our subscribers table
    ...
    if (existingSubscriber && existingSubscriber.subscribed) {
      logStep("Found active subscriber in database", { email: user.email, tier: existingSubscriber.subscription_tier });
    ...
    // If no subscriber record, check Stripe
    ...
    // Update both database and user metadata
    await supabaseClient.from("subscribers").upsert({
      email: user.email,
      user_id: user.id,
