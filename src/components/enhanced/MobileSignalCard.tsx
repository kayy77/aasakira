
import React from 'react';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { 
  Crown, 
  Target, 
  HelpCircle,
  Play,
  Lock,
} from 'lucide-react';
import { Signal } from '@/services/signalService';

interface MobileSignalCardProps {
  signal: Signal;
  isPremium: boolean;
  onExplain: (signal: Signal) => void;
  onReplay: (signal: Signal) => void;
}

const MobileSignalCard: React.FC<MobileSignalCardProps> = ({ 
  signal, 
  isPremium, 
  onExplain, 
  onReplay 
}) => {
  const isHighQuality = signal.confidence >= 75 && ['Smart_Money', 'Multi_Confluence'].includes(signal.strategy);
  
  return (
    <Card className={`glass-card hover-glow border-2 transition-all duration-300 ${
      isHighQuality ? 'border-gold-500/50 shadow-gold-500/20' : 'border-purple-500/30'
    }`}>
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Badge className={`text-xs ${
              signal.type === 'BUY' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'
            } border-0`}>
              {signal.type}
            </Badge>
            <span className="font-bold text-white text-sm">{signal.pair}</span>
            {isHighQuality && (
              <Badge className="bg-gold-500/20 text-gold-400 border-gold-500/30 text-xs">
                <Crown className="w-2 h-2 mr-1" />
                PRO
              </Badge>
            )}
          </div>
          <Badge className={`border-0 text-xs ${
            signal.confidence >= 80 ? 'bg-green-500/20 text-green-400' :
            signal.confidence >= 65 ? 'bg-yellow-500/20 text-yellow-400' :
            'bg-red-500/20 text-red-400'
          }`}>
            {signal.confidence}%
          </Badge>
        </div>
      </CardHeader>
      
      <CardContent className="space-y-3">
        {/* Mobile-Optimized Entry Details */}
        <div className="grid grid-cols-1 gap-2 text-sm">
          <div className="flex justify-between items-center p-2 bg-gray-800/30 rounded">
            <span className="text-gray-400">Entry</span>
            <span className="text-white font-bold">{Number(signal.entry).toFixed(signal.pair.includes('JPY') ? 3 : 5)}</span>
          </div>
          <div className="flex justify-between items-center p-2 bg-gray-800/30 rounded">
            <span className="text-gray-400">Stop Loss</span>
            <span className="text-red-400 font-bold">{Number(signal.stopLoss).toFixed(signal.pair.includes('JPY') ? 3 : 5)}</span>
          </div>
          <div className="flex justify-between items-center p-2 bg-gray-800/30 rounded">
            <span className="text-gray-400">Take Profit</span>
            <span className="text-green-400 font-bold">{Number(signal.takeProfit).toFixed(signal.pair.includes('JPY') ? 3 : 5)}</span>
          </div>
        </div>

        {/* Strategy & Risk - Mobile Layout */}
        <div className="flex items-center justify-between">
          <Badge variant="outline" className="border-purple-500/30 text-purple-400 text-xs">
            {signal.strategy.replace('_', ' ')}
          </Badge>
          <Badge variant="outline" className={`border-0 text-xs ${
            signal.risk === 'Low' ? 'bg-green-500/20 text-green-400' :
            signal.risk === 'Medium' ? 'bg-yellow-500/20 text-yellow-400' :
            'bg-red-500/20 text-red-400'
          }`}>
            {signal.risk} Risk
          </Badge>
        </div>

        {/* Analysis - Mobile Optimized */}
        <div className="space-y-2">
          <div className="text-gray-300 text-xs leading-relaxed">
            {isPremium || !isHighQuality ? signal.analysis : 
              <div className="flex items-center gap-2 text-gray-500">
                <Lock className="w-3 h-3" />
                <span className="text-xs">Upgrade for full analysis</span>
              </div>
            }
          </div>
        </div>

        {/* Mobile Action Buttons */}
        <div className="grid grid-cols-2 gap-2 pt-2">
          <Button
            onClick={() => onExplain(signal)}
            variant="outline"
            size="sm"
            className="border-purple-500/30 hover:bg-purple-500/20 text-xs h-8"
            disabled={!isPremium && isHighQuality}
          >
            <HelpCircle className="w-3 h-3 mr-1" />
            Why?
          </Button>
          <Button
            onClick={() => onReplay(signal)}
            variant="outline" 
            size="sm"
            className="border-blue-500/30 hover:bg-blue-500/20 text-xs h-8"
            disabled={!isPremium && isHighQuality}
          >
            <Play className="w-3 h-3 mr-1" />
            Replay
          </Button>
        </div>
      </CardContent>
    </Card>
  );
};

export default MobileSignalCard;
